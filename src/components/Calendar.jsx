...
import "./Calendar.css";
import {
  FiPlus, FiCalendar, FiUser, FiBell, FiSettings,
  FiFolder, FiClipboard, FiBarChart2
} from "react-icons/fi";
import { BsPersonWorkspace } from "react-icons/bs";
  ...
  const renderMonthView = () => {
    ...
    return (
      <>
        <div className="calendar-weekdays">
          {daysOfWeek.map((day, index) => (
            <div key={day} className={classNames("calendar-weekday", {
              weekend: index === 0 || index === 6,
            })}>
              {day}
            </div>
        ...
        <div className="calendar-grid">
          {dates.map((dateObj, index) => {
            ...
            return (
              <div key={index} className={classNames("calendar-cell", {
                today: isToday,
                otherMonth: !isCurrentMonth,
                "high-density": dayEvents.length >= 3,
              })}>
  ...
  const renderDayView = () => {
    ...
    return (
      <div className="calendar-day-view">
        <div className="day-header">
          <h2>{date.format("dddd, MMMM D, YYYY")}</h2>
          <button onClick={() => openEventModal(date)} className="add-event-button">
            + Add Event
          </button>
        </div>
        <div className="day-grid-container table-layout">
          <table className="day-view-table">
            <tbody>
              {[...Array(24)].map((_, hour) => {
                const hourStart = dayjs(date).hour(hour).minute(0);
                const eventsInHour = dayEvents.filter(event =>
                  dayjs(event.startDateTime).hour() === hour
                );
                return (
                  <tr key={hour}>
                    <td className="hour-label">{hourStart.format("h A")}</td>
    ...
    return (
      <div className="calendar-year-view">
        {months.map((month, i) => {
          const monthDates = getMonthDates(month);
          return (
            <div key={i} className="year-month-box" onClick={() => {}}>
              <h4>{month.format("MMMM")}</h4>
              <div className="mini-calendar-grid">
                {monthDates.map((date, idx) => {
                  const isToday = date.isSame(dayjs(), "day");
